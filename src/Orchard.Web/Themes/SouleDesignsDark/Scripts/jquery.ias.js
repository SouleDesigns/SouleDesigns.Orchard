...
(function($) {
    $.ias = function(options)
    {
        ...
        function paginate(curScrOffset, onCompleteHandler)
        {
            ...
            if (opts.loadMore)
                show_loadmore(urlNextPage, onCompleteHandler);
            else
                _self.doPaginate(urlNextPage, onCompleteHandler);
        }

        /**
         * Load the items from the next page, called from the load more button,
         * split off from the above "Paginate" function to allow explicit 
         * loading of more items rather than the default loading at scroll
         */
        this.doPaginate = function (urlNextPage, onCompleteHandler) {
            remove_loadmore();
            show_loader();
        ...
        /** 
         * Return the load more button or create one
         *
         * @return object load more button jquery object
         */
        function get_loadmore() {
            var loadMore = $(".ias_loadmore");

            if (loadMore.size() == 0) {
                loadMore = $("<div class='ias_loadmore'><button class='btn btn-primary' name='loadMoreButton'>Load More</button></div>");
                loadMore.hide()
                loadMore.on("click", function () { _self.doPaginate(urlNextPage, onCompleteHandler); })
            }
        }
        ...
        /**
         * Inserts the load more and does a fadeIn.
         *
         * @return void
         */
        function show_loadmore(urlNextPage, onCompleteHandler) {
            var loadMore = $(".ias_loadmore");
