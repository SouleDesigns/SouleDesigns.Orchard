    ...
    public class BlogMLBlogAssembler : IBlogAssembler {
        ...
        private void GetPosts(Blog blog, BlogMLBlog blogMLBlog) {
            foreach (var blogMLPost in blogMLBlog.Posts) {
                ...
                if (post.HasExcerpt)
                    post.Excerpt = new Content { Type = Content.TypeHTML /*blogMLPost.Excerpt.ContentType.ToString().ToLowerInvariant()*/, Value = blogMLPost.Excerpt.Text };
                ...
                foreach (BlogMLComment blogMLComment in blogMLPost.Comments) {
                    ...
                    comment.Content = new Content { Type = Content.TypeHTML, Value = blogMLComment.Content.Text };
                ...
                post.Content = new Content { Type = Content.TypeHTML, Value = blogMLPost.Content.Text };                
                post.DateCreated = blogMLPost.DateCreated;
                post.DateModified = blogMLPost.DateModified;
                post.HasExcerpt = blogMLPost.HasExcerpt;

                if (post.HasExcerpt)
                    post.Excerpt = new Content { Type = Content.TypeHTML, Value = blogMLPost.Excerpt.Text };
